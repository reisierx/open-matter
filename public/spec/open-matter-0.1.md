...
**Filename (reserved):** `open-matter.yaml`
**MIME type:** `application/yaml`

This document is the normative definition of open-matter/0.1. Implementations that want to interoperate must follow the sections marked **must**. Everything else is advice.

---

## 1. What this is

open-matter is a convention for putting a small, machine-readable index card *inside the file that circulates*. Version 0.1 defines the **PDF profile**: a standard PDF file attachment. Later versions may add profiles for other formats. The card, not the container, is the point.
...
Readers **must** also accept a card named `agent-frontmatter.yaml` whose `spec` is `pdf-frontmatter/0.1` — the name this convention had on its first day. Writers **must** write `open-matter.yaml` and `spec: open-matter/0.1`.
...
```yaml
spec: open-matter/0.1        # required, exact string
...
1. Look up the attachment by the reserved name `open-matter.yaml`. Do not guess other names.
...
**DocLang.** A representation: what the machine-readable version of a document looks like. open-matter is the carrier: where that representation (or a pointer to it) lives so it cannot be orphaned. Complementary. The `derived` key is the bridge.
...
This version is `open-matter/0.1`. A future version will use a new `spec` string. Readers that do not recognise the `spec` value **must** ignore the card and fall back.
