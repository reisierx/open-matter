...
Reference reader and writer for **open-matter/0.1** — a one-kilobyte YAML card attached to a PDF so a later read can skip the parse.

The standard is vendor-neutral. This package is one implementation.

## Install

Until v0.1.0 is on npm, use the package from this repo (`packages/open-matter`).

```bash
# after publish
npm install open-matter
```
...
```ts
import { writeManifest } from "open-matter";
...
The file stays a valid PDF. Viewers that list attachments will show `open-matter.yaml`.
...
Spec: [`spec/open-matter-0.1.md`](../../spec/open-matter-0.1.md) (CC0). This code is MIT.
